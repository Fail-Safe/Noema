...
| MCP HTTP | Integration | Both transports initialize and serve calls; auth, TLS, host validation, shutdown, and federation-mode restrictions match | `http_smoke.sh` covers both builds plus Rust repeated dual-stack static listeners and dynamic local-address reconciliation; `federation_tls.py` covers authenticated HTTPS, custom CA, and plaintext refusal; `tls_certificate_lifecycle.py` covers validity gates, CLI precedence, monitor bands, and redaction; `mcp_contract.py` covers every publish-mode mutation guard, local stdio exemption, and subscribe-mode publication guards |
...
| Consolidation | Deterministic + bounded real-model integration | Fake LLM covers election, gating, clustering, success/failure events, and idempotency; real model preserves planted synthetic facts and rejects unrelated buckets | Rank/election/watchdog/cadence, heuristic promotion, graduation, all three model profiles, CLI overrides/dry-run/JSON, distilled lineage/telemetry, malformed/offline fallback, restart idempotency, signed bidirectional replay, seven-run baseline comparison, and four-run blinded adversarial comparison automated; Rust prompt/parser quality gaps remain |
...
release replacement: real power-loss durability, corrupt-database salvage
policy, takeover by older Go binaries, pixel-level and multi-emulator human TUI
validation, closure of the observed Rust distillation-quality gap followed by
representative multi-model/multi-reviewer evaluation, and staged packaging /
rollback policy remain. Automatic restore reconciliation is deliberately not
a goal because ambiguous or tampered state requires an explicit operator
choice.
