...
| Fault tolerance | Fault injection | Interrupted writes, malformed Markdown, corrupt DB, unavailable endpoints, and lock contention fail safely | `background_lock_contention.py` covers MCP-only losers, killed-owner release, and replacement acquisition; `io_failure_safety.py` covers pre-mutation trace, lifecycle, and manifest I/O refusal with unchanged bytes/DB/events; Rust unit tests inject returned SQLite aborts after local and replay filesystem mutations; three subprocess tests `SIGKILL` create/replace/move after the filesystem mutation and require idempotent next-open recovery; destructive lifecycle paths, mixed-runtime takeover, power-loss qualification, and corrupt-DB recovery remain pending |
...
Pixel-level/live-terminal TUI validation, destructive-lifecycle crash and
corrupt-DB fault injection, mixed-runtime recovery takeover, and broader
