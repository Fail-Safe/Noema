...
enum MemoryCommand {
    ...
    /// Apply a hash-pinned, insert-only Markdown normalization plan
    NormalizeMarkdown {
        /// JSON plan generated by the Obsidian body-tag doctor
        plan: PathBuf,
        /// Apply the plan; the default is a read-only preflight
        #[arg(long)]
        apply: bool,
        /// Apply without an interactive confirmation prompt
        #[arg(short = 'y', long, requires = "apply")]
...
fn memory_command(cx: &Cortex, command: MemoryCommand) -> Result<()> {
    match command {
        ...
        MemoryCommand::NormalizeMarkdown {
            plan,
            apply,
            yes,
            output,
        } => {
            let bytes = fs::read(&plan).with_context(|| {
                format!("reading markdown normalization plan {}", plan.display())
            })?;
            let plan: crate::markdown_normalization::MarkdownNormalizationPlan =
                serde_json::from_slice(&bytes).context("parsing markdown normalization plan")?;
            let preview = cx.markdown_normalization_preview(&plan)?;
            if output == "json" && !apply {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&serde_json::json!({
                        "schema_version": 1,
                        "operation": "normalize_markdown",
                        "dry_run": true,
                        "traces": preview,
                    }))?
                );
            ...
            if output == "text" {
                println!("Markdown normalization preflight");
                println!("  Traces: {}", preview.len());
                for item in &preview {
                    println!(
                        "  {} [{}] {} operation(s), {} -> {}",
                        item.path,
                        ...
                        item.before_content_hash,
                        item.after_content_hash
                    );
                }
            }
            if !apply {
                if output == "text" {
                    println!("Preview only; no traces changed.");
                }
                return Ok(());
            }
            if !yes {
                if output == "json" || !io::stdin().is_terminal() {
                    bail!("--yes is required for non-interactive markdown normalization");
                }
                print!("Proceed? [y/N]: ");
                io::stdout().flush()?;
                if !tag_mutation_confirmed(&read_line()?) {
            ...
            let results = cx.normalize_markdown(&plan)?;
            if output == "json" {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&serde_json::json!({
                ...
                println!("Normalized {} trace(s).", results.len());
                for result in results {
                    println!(
                        "  {} recovery artifact: {}",
                        result.preview.trace_id, result.recovery_artifact
                    );
                }
            }
        }
