...
fn pane_areas(area: Rect) -> (Rect, Rect) {
    // The established layout gives the list content 34% of the terminal and places a separate
...
mod tests {
    ...
    #[test]
    fn tier_toggles_search_and_help_match_public_state_transitions() {
        let (_temp, cortex) = fixture();
        let mut model = Model::new(&cortex, "dark").unwrap();
    ...
    #[test]
    fn renderer_preserves_visual_hierarchy_at_fixed_size() {
        let (_temp, cortex) = fixture();
